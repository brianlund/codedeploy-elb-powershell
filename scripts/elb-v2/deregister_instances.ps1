...
if(!$target_Group_list) {
    Write-Error "Must have at least one target group to deregister from"
    exit -1
}
...
foreach ($target_group in $target_group_list) {
    Write-Debug "Deregistering $instanceid from $target_group starts"
    deregister_instance $instanceid $target_group
}
...
foreach ($target_group in $target_group_list) {
    wait_for_state $instanceid $target_group $deregister_state
}
